...
constexpr auto lambda_co_status_check(ParamType &value) {
  static_assert(std::is_integral_v<ParamType>,
                "integral state lambda capture value required");
...
#define lambda_co_begin(X)                   \
  lambda_co_status_check(X);                 \
  auto &coroutine_lambda_internal_state = X; \
  switch (X) {                               \
    case 0:
#define lambda_co_return(X) \
  }                         \
  return X
#define lambda_co_yield(X)                                    \
  coroutine_lambda_internal_state = LAMBDA_CO_CONSTEXPR_LINE; \
  return X;                                                   \
  case LAMBDA_CO_CONSTEXPR_LINE:

template <typename Lambda>
[[nodiscard]] constexpr auto lambda_co_range(
    Lambda lambda, std::size_t skip_ = 0,
    std::optional<std::size_t> length_ = {}, std::size_t stride_ = 1) {
  struct Range {
    Lambda generator;
...
template <typename Lambda>
[[nodiscard]] constexpr auto lambda_co_while_not_empty(Lambda lambda) {
  struct Range {
    Lambda generator;
